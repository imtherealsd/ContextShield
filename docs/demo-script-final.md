...
| **SHOT 2** | **0:12 – 0:48** | Switched to **Pipeline Architecture** tab. Cursor moves through Untrusted Context, Scanner + Moss, Risk Engine, and Gated Gemini. | *"ContextShield is a low-latency security gateway positioned in front of the agent. Inbound context is scanned by deterministic rules and Moss semantic retrieval over enterprise policies. Clear threats and safe inputs resolve without a generative model call, while Gemini is gated behind the deterministic path for ambiguous cases. Downstream agents are strictly isolated from unapproved context."* | Parallel Scanner + Moss block; separate Gated Evaluator vs. Downstream Agent boxes. |
