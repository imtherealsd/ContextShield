...
from backend.app.storage import AuditStore, AuditStorageError, get_audit_store
...
class AuditService:
    ...
    async def record_event(
        ...
        try:
            await self._store.record_event(record)
        except AuditStorageError:
            raise
        except Exception as exc:
            # Never include exception text: database drivers may echo connection
            # details or other sensitive implementation data.
            raise AuditStorageError("Audit storage write failed") from exc
        logger.info(
    ...
    async def get_records(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            return await self._store.get_records(limit=limit)
        except AuditStorageError:
            raise
        except Exception as exc:
            raise AuditStorageError("Audit storage read failed") from exc
    ...
    async def clear(self) -> None:
        ...
        try:
            ...
            raise AuditStorageError("Audit storage cleanup failed") from exc
